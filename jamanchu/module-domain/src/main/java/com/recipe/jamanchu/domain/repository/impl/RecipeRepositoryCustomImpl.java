...
public class RecipeRepositoryCustomImpl implements RecipeRepositoryCustom {
    ...
    if (recipesSearchDTO.getIngredients() != null && !recipesSearchDTO.getIngredients().isEmpty()) {
      for (String ingredient : recipesSearchDTO.getIngredients()) {
        combinedCondition = combinedCondition.or(
            recipeIngredient.riName.likeIgnoreCase("%" + ingredient + "%"));
      }

      // 모든 재료가 포함된 레시피를 찾아야 하므로, 각 재료를 포함하는 레시피 ID를 구합니다.
      for (String ingredient : recipesSearchDTO.getIngredients()) {
        List<Long> ingredientRecipeIds = queryFactory.select(recipe.rcpId)
            .from(recipe)
            .join(recipe.ingredients, recipeIngredient)
            .where(recipeIngredient.riName.likeIgnoreCase("%" + ingredient + "%"))
            .fetch();

        combinedCondition = combinedCondition.and(recipe.rcpId.in(ingredientRecipeIds));
      }
    }

    // 난이도 조건 추가
    if (recipesSearchDTO.getRecipeLevel() != null) {
      combinedCondition = combinedCondition.and(recipe.rcpLevel.eq(recipesSearchDTO.getRecipeLevel()));
    }

    // 요리 시간 조건 추가
    if (recipesSearchDTO.getRecipeCookingTime() != null) {
      combinedCondition = combinedCondition.and(recipe.rcpTime.eq(recipesSearchDTO.getRecipeCookingTime()));
    }

    // 스크랩된 레시피 제외 조건 추가
    if (!scrapedRecipeIds.isEmpty()) {
      combinedCondition = combinedCondition.and(recipe.rcpId.notIn(scrapedRecipeIds));
    }
  ...
  public Page<RecipeEntity> searchOrRecipesQueryDSL(RecipesSearchDTO recipesSearchDTO,
    ...
    if (recipesSearchDTO.getIngredients() != null && !recipesSearchDTO.getIngredients().isEmpty()) {
      ...
      for (String ingredient : recipesSearchDTO.getIngredients()) {
        BooleanExpression singleIngredientCondition = recipeIngredient.riName.likeIgnoreCase("%" + ingredient + "%");
    ...
    // 난이도 조건 추가
    if (recipesSearchDTO.getRecipeLevel() != null) {
      combinedCondition = combinedCondition.or(recipe.rcpLevel.eq(recipesSearchDTO.getRecipeLevel()));
    }

    // 요리 시간 조건 추가
    if (recipesSearchDTO.getRecipeCookingTime() != null) {
      combinedCondition = combinedCondition.or(recipe.rcpTime.eq(recipesSearchDTO.getRecipeCookingTime()));
    }
